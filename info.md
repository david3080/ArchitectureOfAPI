# イントロダクション
ここでは、APIの基礎とAPIがアーキテクチャ探訪につながる潜在能力を紹介します。まずAPIの簡単な定義とプロセス内外での使用方法を説明します。そしてAPIの重要性を示すために、本書を通じて進化する実行例であるカンファレンスシステムのケーススタディを紹介します。プロセス外APIは、単純な3層アーキテクチャを超えた考え方を可能にします。このことを示すために、トラフィックパターンとその重要性を紹介します。段階的にケーススタディの概要を説明していきますので、興味のあるエリアがあればすぐに読み進めることができるようになっています。

また、APIとそれに関連するエコシステムを提示するために、いくつかの重要な成果物を使用します。[C4モデル図](https://www.infoq.com/jp/articles/C4-architecture-model/)を使ってケーススタディを紹介し、アプローチの背後にある具体性とロジックを再検討します。また、アーキテクチャ決定レコード（ADR）の使用法や、ソフトウェアライフサイクル全体で決定を明確に定義する価値について学びます。最後に「場合による」判断が必要な場合に意思決定を行うのに役立つアプローチであるADRガイドラインについて説明します。
...
このように積み上げ式に価値を提供することと新たに登場した技術を組み合わせることで、進化型アーキテクチャのコンセプトが生まれます。進化型アーキテクチャとは、アーキテクチャを積み上げ式で進化させるアプローチであり、スピード感をもって変化させ、変化による悪影響のリスクを低減させることに重点を置きます。この道のりにおいて、APIアーキテクチャに取り組む際に以下のアドバイスを心に留めておいてください。

>「アーキテクトは将来を見据えた戦略的な計画を行いたいものだが、 絶えず変化するソフトウェア開発エコシステムはそれを難しくする。変化や変更は避 けることができない。そのため、我々はそれを活用する必要がある。」
>
