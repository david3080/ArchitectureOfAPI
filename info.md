...
APIとそれに関連するエコシステムを表すため、[C4モデル図](https://www.infoq.com/jp/articles/C4-architecture-model/)を使ってケーススタディを紹介し、アプローチの背後にある仕様と論理を検討します。また、アーキテクチャ決定記録（ADR）の使用とソフトウェアライフサイクル全体にわたる決定を明確に定義することの価値について学びます。最後に「場合による」判断が必要な場合に意思決定を行うのに役立つアプローチであるADRガイドラインについて説明します。
...
<center><b>図2. C4コンテキストダイアグラム2</b></center>
<br>
...
<center><b>図3. C4コンポーネントダイアグラム2</b></center>
<br>
...
- [第1章](./1.md)では、出席者APIの設計と仕様について検討します。また、Attendee APIのパフォーマンスに関するバージョン管理およびモデリングエクスチェンジの重要性を紹介します。
...
### C4コンテクスト図
図1は、C4モデルからC4コンテキストダイアグラムを用いて表現しています。この図の意図は、技術者と非技術者の両方の聴衆のためにコンテキストを設定することです。多くのアーキテクチャーの会話では、低レベルの詳細が直に語られ、高レベルの相互作用のコンテキストが設定されないことがあります。システムコンテキストダイアグラムを間違えることの意味を考えてみてください。アプローチをまとめることで、誤解を修正するために何ヶ月もかかる作業を省くことができるかもしれません。 

### C4コンテナダイアグラム
図1は、カンファレンスシステムの全体像を示していますが、コンテナ図は、アーキテクチャの主要な参加者の技術的な分担を説明するのに役立ちます。C4におけるコンテナとは、「システム全体が動作するために必要なもの」（例：カンファレンスデータベース）と定義されます。コンテナダイアグラムは技術的なものであり、上位のシステムコンテキストダイアグラムの上に構築されます。図2はコンテナ図で、顧客が会議システムとやりとりする様子を詳細に記録しています。

> 図 I-2 の会議アプリケーションコンテナは文書化されている
> を単に焼くだけです。通常、C4コンテナは、より多くの
> コンテナの種類（例：Java Spring Application）の詳細を説明します。
> しかし、本書では技術の具体的な説明は避けます。
> 具体的な解決策を示すのに役立たない限りの利点は
> APIや最近のアプリケーションは、ソリューション空間においてかなりの柔軟性を持っているということです。

### C4コンポーネント図
図3の C4コンポーネントダイアグラムは、各コンテナ内の役割と責任、および内部の相互作用の定義に役立ちます。この図は、コンテナの詳細が照会されたときに有用であり、また、コードベースへの非常に有用なマップを提供します。新しいプロジェクトで初めて作業を始めるときのことを考えてみてください。自己記録されたコードベースを閲覧するのも一つの方法ですが、すべてをつなぎ合わせるのは難しいかもしれません。コンポーネント図は、あなたがソフトウェアを構築するために使用している言語/スタックの詳細を明らかにします。技術にとらわれないために、私たちはパッケージ／モジュールという言葉を使いました。

## アーキテクチャ決定記録の使用
開発者、建築家、そして人間として、私たちは皆、"彼らは何を考えているのだろう?「イギリスのリーズとマンチェスターを結ぶM62を走ったことがある人は、高速道路の建設に困惑したことがあるかもしれません。3車線の高速道路が坂を上るにつれて、交通の流れから外れていき、やがて馬車に挟まれた15エーカーほどの農地にスコット・ホール・ファームが姿を現す。地元の伝説によると、この土地の所有者は頑固で、土地を動かすことも譲ることも拒んだので、エンジニアはただ彼の周りに建てたという。その50年後、あるドキュメンタリー番組で、その本当の理由が明らかになった。

というのは、この土地の地下には地質学的な断層があり、高速道路はそのように建設されなければならなかったからです。このように、「なぜ、そうなったか」を推理すると、噂やユーモア、批判が生まれる。
ソフトウェア・アーキテクチャでは、構築しなければならない多くの制約があるため、意思決定を記録し、透明性を確保することが重要である。ADRは、ソフトウェア・アーキテクチャにおける決定を明確にするのに役立つ。
プロジェクト期間中、最も追跡しにくいことの1つは、ある決定の背後にある動機である。プロジェクトに新しく参加した人は、過去の決定に戸惑い、困惑し、喜び、あるいは憤慨するかもしれません。 
-ADRコンセプトの生みの親であるマイケル・ナイガード（Michael Nygard）氏
ADRには、ステータス、コンテキスト、決定、結果の4つの重要なセクションがあります。ADRは提案された状態で作成され、議論に基づいて、通常、受諾または拒否される。また、その決定が後に新たなADRに取って代わられることもあり得ます。コンテキストは、場面を設定し、決定が行われる問題や境界を説明するのに役立ちます。ADRに先立ってブログ記事を作成し、ADRからリンクすることは、コミュニティがあなたの仕事をフォローするのに役立ちますが、コンテキストはブログ記事または詳細な説明を意図したものではありません。決定は、あなたが何をどのように行う予定なのかを明確に示しています。すべての決定は、建築において結果またはトレードオフを伴い、これらは時に間違うととてつもなく高くつくことがあります。 
ADRをレビューする際には、ADRの決定に同意するか、代替的なアプローチがあるかどうかを確認することが重要です。検討されていない代替アプローチがあると、ADRが拒否される可能性があります。却下されたADRには多くの価値があり、ほとんどのチームは、視点の変化を捉えるためにADRを不変にしておくことを選択します。ADRは、主要な参加者が閲覧し、コメントし、ADRを受け入れに移行させることができる場所に提示されると、最も効果的に機能します。

よく聞かれるのが、「どの時点で、チームがすべきなのか」ということです。
ADRを作る？議論されたことを確認することは有用である。
ADRに先立ち、この記録は集団的思考の結果である。
をチーム内で共有することができます。ADRをより広いコミュニティに公開することで、直属のチームを超えたフィードバックの機会を得ることができます。

出席者数 エボリューション ADR
図 I-4 では、会議システムのアーキテクチャを進化させるという決断をしました。これは大きな変更であり、ADRが必要である。表 I-1 は、会議システムを所有するエンジニアリングチームが提案したであろう ADR の例である。

表I-1.ADR001 レガシー会議システムから出席者を切り離す

コンテキスト コンファレンスオーナーからの要望で、現在のコンファレンスの主要な特徴を追加しました。
会議システムは、モバイルアプリへの対応や外部CFPシステムとの連携など、現行システムを維持したまま進化させる必要があります。会議システムは、モバイルアプリケーションと外部CFPシステムとの統合をサポートするように進化させる必要があります。モバイルアプリも外部CFPシステムも、サードパーティーのサービスにアクセスできるようにする必要がある。
決定 我々は、図 I-4 に示すような進化的なステップを踏んで、出席者を分割する。
コンポーネントをスタンドアロンサービスに組み込むことができます。これにより、Attendees サービスに対して API-First 開発を行い、レガシーコンファレンスサービスから API を呼び出すことができるようになります。また、Attendees サービスに直接アクセスし、ユーザー情報を外部の CFP システムに提供する設計もサポートします。
その結果、アテンダントサービスへの呼び出しはプロセスから外れることはなく、以下のような遅延を引き起こす可能性があります。
をテストする必要がある。出席者サービスは、アーキテクチャの障害となる可能性があり、また、出席者サービスの実行による潜在的な影響を軽減するための措置を講じる必要がある可能性があります。
を1つのAttendeesサービスに統合する必要があります。Attendeeサービスが複数の消費者モデルを計画している場合、設計、バージョン管理、およびテストを確実に行い、偶発的な変更を減らす必要があります。

ADRにおけるいくつかの結果は、かなり大きなもので、間違いなくさらなる議論が必要です。いくつかの結果については、後の章に譲ることにします。
APIを使いこなす。ADRガイドライン
Mastering API Architectureの中で、私たちはADRガイドラインを提供し、私たちがカバーしているトピックについて決定を下す際に尋ねるべき重要な質問を列挙するのに役立てる。APIベースのアーキテクチャについて決断を下すのは本当に難しいことで、多くの状況で答えは「場合による」。ADRガイドラインは、文脈なしに "dependence "と言うのではなく、"dependence "が何に依存するかを説明し、あなたの決断を促すのに役立つだろう。ADRガイドラインは、特定の問題に直面したときに、立ち戻ったり、先を読んだりするための参照ポイントとして使用することができます。表I-2に、ADRガイドラインのフォーマットと、ADRガイドラインから期待されることを概説します。 
表I-2.ADRガイドライン形式
決定事項 本書の内容を検討する際に必要と思われる決定事項を記述しています。
ディスカッションのポイント このセクションでは、意思決定の際に持つべき重要なディスカッションを確認することができます。
あなたのAPIアーキテクチャについて。
このセクションでは、意思決定に影響を与えたと思われるあなたの経験をいくつか明らかにします。このセクションでは、あなたの意思決定プロセスに影響を与えたと思われる、あなたの経験の一部を明らかにします。
ADR を作成する際に考慮すべき事項として、具体的な提言を行い、その内容を説明する。
また、なぜこのような勧告を行うのか、その理由も説明します。
