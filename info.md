# はじめに
ここではAPIの基礎とアーキテクチャ探訪への道の一部を発見できればと思います。APIの簡単な定義とインプロセスAPIとアウトオブプロセスAPIについて説明します。APIの重要性を示すために、本書を通じて進化させていく事例として会議システムを紹介します。アウトオブプロセスAPIは単純な3層のアーキテクチャを超えた考え方を可能にします。これを示すためにトラフィックパターンとその重要性を紹介します。段階的にケーススタディの概要を説明しますので興味のあるエリアがあればすぐに読み進めることができるようになっています。

APIとそれに関連するエコシステムを表すため、[C4 モデル図](https://www.infoq.com/jp/articles/C4-architecture-model/)を使って事例を紹介し、アプローチの背後にある仕様と論理を検討します。また、アーキテクチャ決定記録（ADR）の使用と、ソフトウェアライフサイクル全体にわたる決定を明確に定義することの価値について学びます。最後に、「場合による」判断が必要な場合に意思決定を行うのに役立つアプローチであるADRガイドラインについて説明します。
...
## 実行例会議システム事例
本書では会議システムをモデル化することをケーススタディとして選びました。というのも、このドメイン(業務領域)は容易に認識することができますが、進化的なアーキテクチャをモデル化するには十分な複雑さを備えているからです。下図では、会議システムをトップレベルで可視化し、アーキテクチャのコンテキストを議論の土台に置きます。このシステムは、外部の利用者がアカウントを作成し、会議セッションを確認し、出席を予約するために使用されます。

![会議システムのコンテキストダイアグラム1](./img/1.png)

この会議システムのボックスを拡大してみましょう。

![会議システムのコンテキストダイアグラム2](./img/2.png)

会議システムを拡大すると，その主要な技術的構成要素についてより詳細な情報を得ることができます。利用者はWebアプリと対話し、Webアプリは会議アプリのAPIを呼び出します。会議アプリはSQLを使用してバックエンドのデータストアに問い合わせを行います。

APIの観点から、最も興味深い機能が会議アプリのコンテナ内にあることを明らかに している。下図は、この特定のコンテナにズームインし、構造と相互作用を調べることができる。

![会議システムのコンテキストダイアグラム3](./img/3.png)

現在のシステムには、4つの主要コンポーネントとデータベースが関わっています。APIコントローラはUIから入ってくるすべてのトラフィックに直面し、システム内のどこにリクエストをルーティングするかを決定する。このコンポーネントはまた、ネットワークレベルの表現からコード内のオブジェクトや表現へのマーシャリングを担当する。APIコントローラコンポーネントはプロセス内ルーティングの観点から、ジャンクションポイントやフロントコントローラパターンとして機能することに興味をそそられる。APIリクエストと処理にとって、これは重要なパターンです。すべてのリクエストはコントローラを通過し、コントローラはリクエストがどこに向けられるかを決定します。第3章では、コントローラをプロセスの外に持ち出す可能性について見ていきます。 
Attendee、Booking、Sessionの各コンポーネントは、リクエストをクエリに変換し、プロセス外でデータベースに対してSQLを実行することに関与しています。既存のアーキテクチャでは、データベースは重要なコンポーネントであり、例えば予約とセッションの間の制約など、関係を強制する可能性があります
